...
VALUES
    ...
    ('21 堂組合包方案',4800,21); --名稱為 21 堂組合包方案，價格為4,800 元，堂數為21
...
(
	(SELECT "id" 
       FROM "USER" 
       WHERE name='王小明'), --預約人設為王小明
    (select id from "COURSE" where name='重訓基礎課'),
    '2024-11-24 16:00:00',--預約時間 booking_at 設為2024-11-24 16:00:00
	'即將授課' --狀態 status 設定為即將授課
),
(
	(SELECT "id" 
       FROM "USER" 
       WHERE name='好野人'), --預約人設為王小明
    (select id from "COURSE" where name='重訓基礎課'),
    '2024-11-24 16:00:00',--預約時間 booking_at 設為2024-11-24 16:00:00
	'即將授課' --狀態 status 設定為即將授課
);
...
(
	(SELECT "id" 
       ...
       WHERE name='王小明'), --預約人設為王小明
    (select id from "COURSE" where name='重訓基礎課'),
    '2024-11-24 17:10:25',--預約時間 booking_at 設為2024-11-24 17:10:25
	'即將授課' --狀態 status 設定為即將授課
);
...
where
	EXTRACT(MONTH FROM "CREDIT_PURCHASE".created_at)=11
...
select
	SUM("銷售價格" * "銷售數量") as "總營收"
from (
	select
		"CREDIT_PACKAGE"."name" as "組合包方案名稱",
		"CREDIT_PACKAGE".price as "銷售價格",
		COUNT(*) as "銷售數量"
	from
		"CREDIT_PACKAGE"
	inner join 
